...
#SBATCH -J Biggan_PF_uniform           # 所运行的任务名称 (自己取)
...
#SBATCH -o Biggan_PF_uniform      # 打印输出的文件
...
do
  ...
  --sparse --sema --sparse_init pruning --pruning_mode uniform_G --pruning_rate $pruning_rate --resume  \
...
do
  python train_prune_finetune.py \
  --shuffle --batch_size 50 --parallel \
  --sparse --sema --sparse_init pruning --pruning_mode uniform_GD --pruning_rate $pruning_rate --resume  \
  --experiment_name unbalanced_BigGAN_C10_seed1_Gch64_Dch64_bs50_nDs4_Glr2.0e-04_Dlr2.0e-04_Gnlrelu_Dnlrelu_GinitN02_DinitN02_ema \
  --num_G_accumulations 1 --num_D_accumulations 1 --num_epochs 500 \
  --num_D_steps 4 --G_lr 2e-4 --D_lr 2e-4 \
  --dataset C10 \
